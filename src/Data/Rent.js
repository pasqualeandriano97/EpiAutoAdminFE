...
export const getRentsBydate = (token, date) => {
  return fetch("http://localhost:3001/rent/date?date=" + date, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
...
export const getPreventive = (token, plate, email, body) => {
  return fetch(
    "http://localhost:3001/rent/admin?email=" + email + "&plate=" + plate,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
...
export const saveRent = (token, userId, payload) => {
  return fetch("http://localhost:3001/rent/save/admin?id=" + userId, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
